...
typedef struct
{
  ...
  byte PSL[4],SSL[4];                /* Current slots          */
  byte PSLReg,SSLReg[4];             /* Slot registers         */

  void *Chunks[MAXCHUNKS];           /* Memory blocks to free  */
  int NChunks;                       /* Number of memory blcks */

  char *SRAMName[MAXSLOTS];          /* SRAM filenames         */
  byte SaveSRAM[MAXSLOTS];           /* Save SRAM on exit      */
  byte *SRAMData[MAXSLOTS];          /* SRAM data              */
...
static MsxLazyState *GMSX = 0;

static int EnsureMsxLazyState(void)
{
  if(GMSX) return(1);
  GMSX = (MsxLazyState *)calloc(1,sizeof(MsxLazyState));
  return(GMSX!=0);
}

static void ReleaseMsxLazyState(void)
{
  if(GMSX) free(GMSX);
  GMSX = 0;
}
...
#define RAM        (GMSX->RAM)
#define EmptyRAM   (GMSX->EmptyRAM)
#define SaveCMOS   (GMSX->SaveCMOS)
#define MemMap     (GMSX->MemMap)
#define RAMData    (GMSX->RAMData)
#define RAMMapper  (GMSX->RAMMapper)
#define RAMMask    (GMSX->RAMMask)
#define ROMData    (GMSX->ROMData)
#define ROMMapper  (GMSX->ROMMapper)
#define ROMMask    (GMSX->ROMMask)
#define ROMType    (GMSX->ROMType)
#define EnWrite    (GMSX->EnWrite)
#define PSL        (GMSX->PSL)
#define SSL        (GMSX->SSL)
#define PSLReg     (GMSX->PSLReg)
#define SSLReg     (GMSX->SSLReg)
#define Chunks     (GMSX->Chunks)
#define NChunks    (GMSX->NChunks)
#define SRAMName   (GMSX->SRAMName)
#define SaveSRAM   (GMSX->SaveSRAM)
#define SRAMData   (GMSX->SRAMData)
#define Kanji      (GMSX->Kanji)
#define KanLetter  (GMSX->KanLetter)
#define KanCount   (GMSX->KanCount)
#define JoyState   (GMSX->JoyState)
#define MouState   (GMSX->MouState)
#define MouseDX    (GMSX->MouseDX)
#define MouseDY    (GMSX->MouseDY)
#define OldMouseX  (GMSX->OldMouseX)
#define OldMouseY  (GMSX->OldMouseY)
#define MCount     (GMSX->MCount)
#define PPI        (GMSX->PPI)
#define IOReg      (GMSX->IOReg)
#ifndef NO_FLOPPY
#define FDC        (GMSX->FDC)
#define FDD        (GMSX->FDD)
#endif
#define PSG        (GMSX->PSG)
#define OPLL       (GMSX->OPLL)
#define SCChip     (GMSX->SCChip)
#define SCCOn      (GMSX->SCCOn)
#define FMPACKey   (GMSX->FMPACKey)
#define SIO        (GMSX->SIO)
#define RTCReg     (GMSX->RTCReg)
#define RTCMode    (GMSX->RTCMode)
#define RTC        (GMSX->RTC)
...
/* Internal MSX state is heap-backed and allocated lazily. */
...
/* SRAM arrays are part of lazy state. */
...
/** Kanji font ROM *******************************************/
/* Kanji state is part of lazy state. */

/** Keyboard, joystick, and mouse ****************************/
volatile byte KeyState[16];        /* Keyboard map state     */
/* Input state is part of lazy state. */

/** General I/O registers: i8255 *****************************/
/* PPI state is part of lazy state. */

/** Disk controller: WD1793 **********************************/
/* Floppy state is part of lazy state. */

/** Sound hardware: PSG, SCC, OPLL ***************************/
/* Sound chip state is part of lazy state. */

/** Serial I/O hardware: i8251+i8253 *************************/
/* SIO state is part of lazy state. */

/** Real-time clock ******************************************/
/* RTC state is part of lazy state. */
...
MCFEntry *MCFEntries = 0;          /* Entries from .MCF file */

/** Cheat codes **********************************************/
byte CheatsON    = 0;              /* 1: Cheats are on       */
int  CheatCount  = 0;              /* # cheats, <=MAXCHEATS  */
CheatCode *CheatCodes = 0;
...
static byte *GetMemory(int Size)
{
  ...
  if(!GMSX||(Size<=0)||(NChunks>=MAXCHUNKS)) return(0);
  P=(byte *)malloc(Size);
...
static int EnsureMCFStorage(void)
{
  if(MCFEntries) return(1);
  MCFEntries=(MCFEntry *)calloc(MAXCHEATS,sizeof(MCFEntry));
  return(MCFEntries!=0);
}

static int EnsureCheatStorage(void)
{
  if(CheatCodes) return(1);
  CheatCodes=(CheatCode *)calloc(MAXCHEATS,sizeof(CheatCode));
  return(CheatCodes!=0);
}
...
int StartMSX(int NewMode,int NewRAMPages,int NewVRAMPages)
{
  ...
  if(!EnsureMsxLazyState())
  {
    if(Verbose) puts("Not enough memory for MSX runtime state");
    return(0);
  }
  ...
  /* Delay host-side framebuffer/audio allocation until after core RAM setup */
  if(!msx_host_prepare_runtime())
  {
    if(Verbose) printf("Host runtime preparation FAILED\n");
    return(0);
  }
...
void TrashMSX(void)
{
  ...
  if(CheatCodes) { free(CheatCodes);CheatCodes=0; }
  if(MCFEntries) { free(MCFEntries);MCFEntries=0; }
  CheatCount = 0;
  MCFCount   = 0;
  CheatsON   = 0;
...
int SaveCHT(const char *Name)
{
  ...
  if(!CheatCodes || !CheatCount) return(0);
...
int ApplyMCFCheat(int N)
{
  ...
  if(!EnsureCheatStorage()) return(0);

  /* Must be a valid MSX-specific entry */
  if(!MCFEntries || (N<0)||(N>=MCFCount)||
     (MCFEntries[N].Addr>0xFFFF)||(MCFEntries[N].Size>2))
    return(0);
...
byte *LoadROM(const char *Name,int Size,byte *Buf)
{
  ...
  /* Open file or fall back to host-provided assets (BIOS, etc.) */
  F=fopen(Name,"rb");
  if(!F)
  {
    Builtin = msx_host_get_builtin_file(Name,&BuiltinSize);
    if(!Builtin) return(0);

    if(!Size) Size = BuiltinSize;
    if((unsigned int)Size > BuiltinSize) return(0);

    /* Host-provided immutable assets such as BIOS blobs can be used
     * directly when caller requests an owned buffer of exact size. This
     * avoids keeping both the host copy and a second malloc()ed copy. */
    if(!Buf && ((unsigned int)Size == BuiltinSize))
      return((byte *)Builtin);

    P=Buf? Buf:GetMemory(Size);
    if(!P) return(0);
    memcpy(P,Builtin,Size);
    return(P);
  }
...
int LoadCart(const char *FileName,int Slot,int Type)
{
  ...
  MappedROM = msx_host_get_mapped_rom(FileName,&MappedSize);

  /* Try opening file unless host already exposed a mapped ROM */
  F = 0;
  if(!MappedROM)
  {
    if(!(F=fopen(FileName,"rb"))) return(0);
    ...
    printf("Found %s (host-mapped ROM):\n",FileName);

  /* Determine size via file length or mapped ROM size */
  if(MappedROM)
  {
    Len = (int)MappedSize;  /* Keep in bytes; >>13 is applied below uniformly */
  }
  else if(!fseek(F,0,SEEK_END)) Len=ftell(F);
  else
  {
    /* Read file in 16kB increments */
  ...
  /* Check "AB" signature in a file or mapped ROM */
  ROM64=0;
  if(MappedROM)
  {
    C1 = MappedSize > 0 ? MappedROM[0] : 0;
    C2 = MappedSize > 1 ? MappedROM[1] : 0;
  }
  else
  {
    C1=fgetc(F);
  ...
  if((C1!='A')||(C2!='B'))
    ...
    else if(fseek(F,0x4000,SEEK_SET)>=0)
    {
      C1=fgetc(F);
      C2=fgetc(F);
      ROM64=(C1=='A')&&(C2=='B');
  ...
  if((Len>=2)&&((C1!='A')||(C2!='B')))
    if(MappedROM)
    {
      const unsigned int Offset = 0x2000*(Len-2);
      if(MappedSize >= Offset+2)
      {
        C1 = MappedROM[Offset];
        C2 = MappedROM[Offset+1];
      }
    }
    else if(fseek(F,0x2000*(Len-2),SEEK_SET)>=0)
    {
      C1=fgetc(F);
      C2=fgetc(F);
    }
  ...
  if(Verbose) printf("  Cartridge %c in slot %d:%d: ",'A'+Slot,PS,SS);
  ...
  if(Verbose)
    printf("pages=%d mask=%d..",Pages,Pages>0? Pages-1:0);

  /* Assign ROMMask for MegaROMs */
  ROMMask[Slot]=!ROM64&&(Len>4)? (Pages-1):0x00;
  /* Use XIP pointer directly when the ROM fits exactly (no mirroring needed).
   * FreeMemory() is a no-op for untracked (XIP) pointers, so teardown is safe. */
  if(MappedROM && (Pages==Len))
  {
    if(Verbose) printf("using direct host mapping..");
    ROMData[Slot]=P=(byte *)MappedROM;
  }
  else
  {
    ...
    if(MappedROM)
    {
      if(Verbose) printf("copied from host mapping..");
      memcpy(P,MappedROM,Len<<13);
    }
    else if(!LoadROM(FileName,Len<<13,P)) { PRINTFAILED;return(0); }
  ...
  if((Type>=MAP_GUESS)&&(ROMMask[Slot]+1>4))
  {
    ...
    printf("type %s..",Type>=MAP_GUESS? "UNKNOWN":ROMNames[Type]);
  ...
  if(MAP_SRAM(Type))
  {
    ...
    /* Generate SRAM file name and load SRAM contents */
    {
      ...
      SourceName = msx_save_build_path(SavePath,sizeof(SavePath),".sav")
                 ? SavePath
                 : FileName;

      if((SRAMName[Slot]=(char *)GetMemory(strlen(SourceName)+5)))
      {
        /* Compose SRAM file name */
        strcpy(SRAMName[Slot],SourceName);
        if(SourceName==FileName)
        {
          T=strrchr(SRAMName[Slot],'.');
          if(T) strcpy(T,".sav"); else strcat(SRAMName[Slot],".sav");
        }
        if(Verbose) printf("MSX SRAM path: %s..",SRAMName[Slot]);
        /* Try opening file... */
...
int LoadMCF(const char *Name)
{
  ...
  return(0);
#else
  if(!EnsureMCFStorage()) return(0);
  MCFCount = LoadFileMCF(Name,MCFEntries,MAXCHEATS);
  return(MCFCount);
#endif
}
...
int SaveMCF(const char *Name)
{
  ...
  return(0);
#else
  return((MCFEntries&&MCFCount>0)? SaveFileMCF(Name,MCFEntries,MCFCount):0);
#endif
}
