...
static WaveChannel *WaveCH = 0;

static int EnsureWaveChannels(void)
{
  int I;

  if(WaveCH) return(1);

  WaveCH = (WaveChannel *)calloc(SND_CHANNELS,sizeof(WaveChannel));
  if(!WaveCH) return(0);

  for(I=0;I<SND_CHANNELS;++I) WaveCH[I].Type = SND_MELODIC;
  return(1);
}
...
{
  if(!EnsureWaveChannels()) return(0);
