...
static CPCPortRead	*readPorts = NULL;
static int NumWritePorts = 0;
static CPCPortWrite *writePorts = NULL;
static int NumResetFunctions = 0;
static CPC_RESET_FUNCTION *resetFunctions = NULL;

static BOOL AllocateCpcTables(void);


void CPC_InstallResetFunction(CPC_RESET_FUNCTION resetFunction)
{
	if ((resetFunctions==NULL) && (!AllocateCpcTables()))
		return;
...
{
	if ((readPorts==NULL) && (!AllocateCpcTables()))
		return;
...
{
	if ((writePorts==NULL) && (!AllocateCpcTables()))
		return;
...
static unsigned char **ExpansionROMTable = NULL;
...
static unsigned char **ExpansionRomData = NULL;
/* true/false depending on if expansion rom is enabled or not */
static BOOL *ExpansionRomActive = NULL;
...
int    ExpansionRom_SetRomData(const unsigned char *RomData, const unsigned long RomDataSize, const int RomIndex)
{
	...
	(void)RomDataSize;
	(void)RomIndex;
	return ARNOLD_VERSION_UNSUPPORTED;
#else
...
static BOOL AllocateCpcTables(void)
{
	if (readPorts==NULL)
	{
		readPorts = malloc(32 * sizeof(*readPorts));
		if (readPorts==NULL)
		{
			return FALSE;
		}
	...
	if (writePorts==NULL)
	{
		writePorts = malloc(32 * sizeof(*writePorts));
		if (writePorts==NULL)
		{
			free(readPorts);
			readPorts = NULL;
	...
	if (resetFunctions==NULL)
	{
		resetFunctions = malloc(32 * sizeof(*resetFunctions));
		if (resetFunctions==NULL)
		{
			free(writePorts);
			writePorts = NULL;
	...
	if (ExpansionROMTable==NULL)
	{
		ExpansionROMTable = calloc(256, sizeof(*ExpansionROMTable));
		if (ExpansionROMTable==NULL)
		{
			free(resetFunctions);
			resetFunctions = NULL;
	...
	if (ExpansionRomData==NULL)
	{
		ExpansionRomData = (unsigned char **)calloc(17, sizeof(*ExpansionRomData));
		if (ExpansionRomData==NULL)
		{
			free(ExpansionROMTable);
			ExpansionROMTable = NULL;
			free(resetFunctions);
			resetFunctions = NULL;
			free(writePorts);
			writePorts = NULL;
	...
	if (ExpansionRomActive==NULL)
	{
		ExpansionRomActive = (BOOL *)calloc(17, sizeof(*ExpansionRomActive));
		if (ExpansionRomActive==NULL)
		{
			free(ExpansionRomData);
			ExpansionRomData = NULL;
			free(ExpansionROMTable);
			ExpansionROMTable = NULL;
			free(resetFunctions);
			resetFunctions = NULL;
	...
	if (Amstrad_MemoryPages==NULL)
	{
		Amstrad_MemoryPages = (unsigned char **)calloc(32, sizeof(*Amstrad_MemoryPages));
		if (Amstrad_MemoryPages==NULL)
		{
			free(ExpansionRomActive);
			ExpansionRomActive = NULL;
			free(ExpansionRomData);
			ExpansionRomData = NULL;
			free(ExpansionROMTable);
			ExpansionROMTable = NULL;
	...
	if (RamConfigurationTable==NULL)
	{
		RamConfigurationTable = malloc((64*4) * sizeof(*RamConfigurationTable));
		if (RamConfigurationTable==NULL)
		{
			free(ExpansionROMTable);
			ExpansionROMTable = NULL;
...
static void FreeCpcTables(void)
{
	if (readPorts!=NULL)
	{
		free(readPorts);
	...
	if (Amstrad_MemoryPages!=NULL)
	{
		free(Amstrad_MemoryPages);
		Amstrad_MemoryPages = NULL;
	}
	...
	if (RamConfigurationTable!=NULL)
	{
		free(RamConfigurationTable);
		RamConfigurationTable = NULL;
	}
}
...
/*		Memory_Init(); */
		...
		#ifdef GX4000_TRIM_NONPLUS
		CPC_SetRamConfig(0); /* GX4000 profile: keep base 64K only */
		#else
