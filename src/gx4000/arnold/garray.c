...
static void GateArray_BuildGreenDisplayColours(void)
{
	int i;
	...
	for (i=0; i<32; i++)
	{
		...
		DisplayColours[i].u.element.Green = (char)((GreenIntensityInt*GreenFraction)>>16);
	}
}

static void GateArray_BuildGreyDisplayColours(void)
{
	int i;
	...
	for (i=0; i<32; i++)
	{
		...
		DisplayColours[i].u.element.Red = GreyScaleColour.R;
		DisplayColours[i].u.element.Green = GreyScaleColour.G;
		DisplayColours[i].u.element.Blue = GreyScaleColour.B;
	}
}
...
/*		BrightnessControl_AdjustBrightness(&SourceRGB, &DestRGB, Brightness); */
		...
		DisplayColours[i].u.element.Red = DestRGB.R;
		DisplayColours[i].u.element.Green = DestRGB.G;
		DisplayColours[i].u.element.Blue = DestRGB.B;
    }
}
...
void    GateArray_SetMonitorColourMode(MONITOR_COLOUR_MODE      MonitorMode)
{
        ...
        switch (MonitorMode)
        {
                ...
                {
					GateArray_BuildGreenDisplayColours();
                }
                break;

                case MONITOR_MODE_GREY_SCALE:
                {
					GateArray_BuildGreyDisplayColours();
                }
                break;
...
{
	/* Tables are generated on-demand when monitor mode changes. */
}
